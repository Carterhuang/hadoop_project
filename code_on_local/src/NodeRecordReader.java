...
public class NodeRecordReader extends RecordReader<IntWritable, Node> {
    ...
    private IntWritable curKey; // The current key of our NodeRecordReader, the current Node's nodeid, <hh548>: now it is blockID
    ...
    public boolean nextKeyValue() throws IOException {
	    ...
	    //The line needs to be split into 4 pieces: the nodeid, the current PageRank, optional list of outgoing links, blockid
	    String[] pieces = lineValue.toString().split("\\s+");
	    if(pieces.length != 4) {
            System.out.println("piece length is: " + pieces.length);
	        throw new IOException("Given poorly-formatted record: " + lineValue.toString()); 
            //If this isn't what's going on, then we need to report an error.
	    }

	    int[] outs; //Now, we need to determine the outgoing links.
	    if(!pieces[2].equals("#")) { //If there are any
	    ...
	    try {
            ...
            blockId = Integer.parseInt(pieces[3]);     // changed by hh548
	    } catch(NumberFormatException nfe) {
        ...
        curKey = new IntWritable(nodeId);   //     changed by hh548
	    curVal = new Node(nodeId, blockId, outs);
